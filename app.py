...
def render_professional_pnid():
    ...
    for _, equip in REFERENCE_EQUIPMENT.iterrows():
        if equip['type'] == 'instrument':
            ...
            # Equipment symbol
            svg_parts.append(f'<g transform="translate({equip["x"]},{equip["y"]}) scale({SYMBOL_SCALE})">') # Apply scale here
            if equip['type'] in ISA_SYMBOLS:
                svg_parts.append(ISA_SYMBOLS[equip['type']])
            svg_parts.append(f'<text x="{equip["width"]/2}" y="{equip["height"]+15}" text-anchor="middle" font-size="{11/SYMBOL_SCALE}">{equip["tag"]}</text>') # Adjust font size if scaled
            svg_parts.append('</g>')
